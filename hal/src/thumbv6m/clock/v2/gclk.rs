...
//! Functionality:
//!
//! * Provides 8 [Gclks](Gclk) fed by clock sources implementing the
//!   [GclkSource] trait
...
impl<G: GclkId> GclkToken<G> {
    ...
    fn wait_syncbusy(&self) {
        while self.gclk().status.read().syncbusy().bit() {}
    }
    ...
    ///
    /// [`Gclk0`] and [`Gclk3`] to [`Gclk11`] has 8 division factor bits
    /// [`Gclk1`] has 16 division factor bits
    /// [`Gclk2`] has 5 division factor bits
    #[inline]
    fn set_div(&mut self, div: G::DividerType) {
        // Get the divsel and divider values
        let (sel, div) = div.get_inner();
        self.genctrl().modify(|_, w| unsafe {
            w.id().bits(G::NUM);
            w.divsel().bit(sel)
        });
        self.gendiv().modify(|_, w| unsafe {
            w.id().bits(G::NUM);
            w.div().bits(div)
        });
        self.wait_syncbusy();
...
/// selects among the 8 possible GCLKs.
#[allow(missing_docs)]
pub enum DynGclkId {
    Gclk0,
    Gclk1,
...
pub trait GclkId: Sealed {
    ...
    ///
    /// Gclks have different dividers. The [`GclkDivider`] trait abstracts over
    /// these details.
...
/// Type-level variant representing the identity of GCLK2
///
/// This type is a member of several [type-level enums]. See the documentation
/// on [type-level enums] for more details on the pattern.
///
...
pub trait GclkDivider: Sealed + Default + Copy {
    /// Returns divsel value and divider register value
    fn get_inner(&self) -> (bool, u16);
...
/// Enum expressing all possible division factors for [`Gclk2`]
///
/// Represents [`Gclk2`] clock generator divider
///
/// Division is interpreted differently depending on state of `DIVSEL` flag
///
/// In `DIVSEL` mode `DIV1` (register value 0) the division factor is directly
...
/// ```
///
/// The maximum division factor is 64 even though the register could be able
/// to express much larger dividers. Hardware ignores any larger value,
/// effectively constrained at max division factor.
///
/// See the datasheet for more details
...
pub enum Gclk2Div {
    ...
    /// of the clock
    Div(u8),
    /// Provides a division factor of `2^5 = 32`
    Div2Pow5,
    /// Provides a division factor of `2^6 = 64`
    Div2Pow6,
}

impl Sealed for GclkDiv {}
impl Sealed for Gclk1Div {}
impl Sealed for Gclk2Div {}
...
impl GclkDivider for Gclk1Div {
    ...
    fn get_inner(&self) -> (bool, u16) {
        match self {
            // Maximum reach of DIV1 mode is 65535
            Gclk1Div::Div(div) => (false, *div),
            // Set the divider to be 65536
            // 2^(1 + 15) = 65536
            Gclk1Div::Div2Pow16 => (true, 15),
            // Set the divider to be 131072
            // 2^(1 + 16) = 131072
            Gclk1Div::Div2Pow17 => (true, 16),
        }
    }
}

impl GclkDivider for Gclk2Div {
    #[inline]
    fn get_div(&self) -> u32 {
        match self {
            // Maximum reach of DIV1 mode is 31
            Gclk2Div::Div(div) => (*div).into(),
            // Set the divider to be 32
            // 2^(1 + 4) = 32
            Gclk2Div::Div2Pow5 => 32,
            // Set the divider to be 64
            // 2^(1 + 5) = 64
            Gclk2Div::Div2Pow6 => 64,
        }
    }
    ...
    fn get_inner(&self) -> (bool, u16) {
        match self {
            // Maximum reach of DIV1 mode is 31
            Gclk2Div::Div(div) => (false, (*div).into()),
            // Set the divider to be 32
            // 2^(1 + 4) = 32
            Gclk2Div::Div2Pow5 => (true, 4),
            // Set the divider to be 64
            // 2^(1 + 5) = 64
            Gclk2Div::Div2Pow6 => (true, 5),
        }
    }
}
...
impl GclkDivider for GclkDiv {
    ...
    fn get_inner(&self) -> (bool, u16) {
        match self {
            // Maximum reach of DIV1 mode is 255
            GclkDiv::Div(div) => (false, (*div).into()),
            // Set the divider to be 256
            // 2^(1 + 7) = 256
            GclkDiv::Div2Pow8 => (true, 7),
            // Set the divider to be 512
            // 2^(1 + 8) = 512
            GclkDiv::Div2Pow9 => (true, 8),
        }
    }
}
...
impl GclkSourceId for XoscId {
    const DYN: DynGclkSourceId = DynGclkSourceId::Xosc;
}
...
impl GclkSourceId for Osc32kId {
    const DYN: DynGclkSourceId = DynGclkSourceId::Osc32k;
}
impl GclkSourceId for Xosc32kId {
    const DYN: DynGclkSourceId = DynGclkSourceId::Xosc32k;
}
impl GclkSourceId for Osc8mId {
...
impl GclkSourceId for DpllId {
    const DYN: DynGclkSourceId = DynGclkSourceId::Dpll;
}
